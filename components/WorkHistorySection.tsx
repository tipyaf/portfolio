...
export default function WorkHistorySection({ jobs, locale }: WorkHistoryProps) {
  const t = useTranslations('workHistory');
  const jobsViews = useJobs(jobs, locale);
  ...
  return (
    ...
    >
      <h2 className="heading text-tertiary">{t('title')}</h2>
        ...
        <div className="flex w-full flex-col flex-wrap gap-16">
          {jobsViews.map((item) => (
            <motion.div
              ...
              key={item._key}
              initial={{ scale: 1 }}
              animate={{ scale: selectedId === item._key ? 1.2 : 1 }}
              transition={{ duration: 0.3 }}
              whileHover={{ scaleY: 1.1 }}
            >
              <JobCard showDescription={false} job={item} locale={locale} />
            </motion.div>
          ))}
        </div>
